...
def digest(text) :
  ...
  # calls server here
  global client
  #webapp need start NLPclient everytime
  #when testing dataset, should not start client everytime
  #if not client:
  client = NLPclient()  
...
def answer_quest_nonenglish(q,talker) :
  ...
  q_lemmas=[]
  answerer = Talker(from_text=q, params=talker.params)
...
def answer_quest_ripple(q,talker) :
  '''
  given question q, interacts with talker and returns
  its best answers
  '''
...
class Talker :
  ...
  def __init__(self,
    ...
    print("starttime Local time:", local_time)
    '''
    ...
    if from_pdf:
       ...
       self.from_file=from_file 
       if self.params.stanza_parsing == True:
    ...
    print("endParseTime Local time:", local_time)
    '''

    self.client = client
    self.totalSents = len(self.db[0])
    self.totalWords = get_totalWords(self.db)
    ...
    print("doneSumkeysTime Local time:", local_time)
    '''
